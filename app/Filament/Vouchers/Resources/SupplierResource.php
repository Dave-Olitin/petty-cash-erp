...
class SupplierResource extends Resource
{
    ...
    public static function getEloquentQuery(): Builder
    {
        return parent::getEloquentQuery()
            ...
            ->withSum(
                ['purchaseEntries as total_paid' => fn ($q) => $q->where('entry_type', '!=', 'return')],
                'amount_paid'
            )
            ->addSelect([
                'net_balance' => \App\Models\PurchaseEntry::selectRaw('COALESCE(SUM(CASE WHEN entry_type != "return" THEN COALESCE(balance_due, grand_total - amount_paid, 0) ELSE 0 END), 0)')
                    ->whereColumn('tax_registration_id', 'tax_registrations.id')
            ])
            ->withCount(
                ['purchaseEntries as open_invoices' => fn ($q) => $q->where('entry_type', '!=', 'return')->where('payment_status', '!=', 'paid')]
            );
    }
