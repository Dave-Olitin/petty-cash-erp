...
class VoucherObserver
{
    ...
    public function updated(Voucher $voucher): void
    {
        ...
        if ($voucher->type === 'petty_cash' && $voucher->wasChanged('status') && $voucher->status === VoucherStatus::Paid->value) {
            ...
            // Wrap both sum() reads in a single transaction so they execute against a
            // consistent snapshot — prevents a race where another voucher is paid between
            // the two queries, producing an incorrectly high balance reading.
